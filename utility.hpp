...
void ChooseSortMethod(vector<int> ar, int size){
  cout << "Now choose sorting alghorithm:\n1 - selection sort\n2 - bubble sort\n3 - merge sort\n4 - paste sort\n5 - quick sort\n6 - shell sort\n7 - shaker sort\n8 - heap sort\n";
  int choice;
  cin >> choice;
...
void ChooseSearchMethod(vector<int> ar, int size){
  int target;
  cout << "Enter target: ";
  cin >> target;
  cout << "Now choose search method:\n1 - linear search\n2 - linear search with barrier\n3 - binary search\n";
...
void ChooseOperation(vector<int> ar, int size){
  cout << "What you`ll do?\n1 - sort\n2 - search\n3 - operate a hash table\n";
  int option;
  cin >> option;
  switch (option) {
...
vector<int> ArrGenV(int size, int cap){
  cout << "Enter seed: ";
  int seed;
  cin >> seed;
  srand(seed);
  ...
  vector<int> ar;
  for(int i = 0; i<size; i++) ar[i] = rand()%cap;
  return ar;
}
